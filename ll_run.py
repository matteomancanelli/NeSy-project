    ...
    for current_sample_size in range(SAMPLE_SIZE_START_VALUE, SAMPLE_SIZE_START_VALUE + SAMPLE_SIZE_INCREMENT * SAMPLE_SIZE_INCREMENT_ITERATIONS, SAMPLE_SIZE_INCREMENT):            
        ...
        for i_form, formula in enumerate(formulas):
            ...
            for exp in range(N_EXPERIMENTS_PER_FORMULA):
                if i_form!=0 or current_sample_size!=1000:
                    continue
                ...
                ########################################################
                # Experiment RNN+BK (with logic loss only)
                ########################################################
                ...
                for current_prefix_len in range(PREFIX_LEN_START_VALUE, PREFIX_LEN_START_VALUE + PREFIX_LEN_INCREMENT * PREFIX_LEN_INCREMENT_ITERATIONS, PREFIX_LEN_INCREMENT):
                    ...
                    # Save the model
                    model_file = pathlib.Path("ll_models", f"model_rnn_bk_formula_{i_form}_sample_size_{current_sample_size}_exp_{exp}_prefix_len_{current_prefix_len}.pt")
            ...
            results_config_json_file = pathlib.Path("results", "results_ll.json")
    ...
    for config in range(N_CONFIGURATIONS):
        ...
        for current_sample_size in range(SAMPLE_SIZE_START_VALUE, SAMPLE_SIZE_START_VALUE + SAMPLE_SIZE_INCREMENT * SAMPLE_SIZE_INCREMENT_ITERATIONS, SAMPLE_SIZE_INCREMENT):
            ...
            for i_form, formula in enumerate(DNF_formulas_infix):
                ...
                for exp in range(N_EXPERIMENTS_PER_FORMULA):
                    # Models
                    rnn_bk = LSTM_model(HIDDEN_DIM, NVAR + 1, NVAR + 1)
                    ...
                    with open(results_config_file, "a") as f:
                        ...
                        for current_prefix_len in range(PREFIX_LEN_START_VALUE, PREFIX_LEN_START_VALUE + PREFIX_LEN_INCREMENT * PREFIX_LEN_INCREMENT_ITERATIONS, PREFIX_LEN_INCREMENT):
                            ...
                            f.write("train acc next activity:\nRNN+BK:{}\tRNN+BK Greedy:{}\n".format(
                                mean(formula_experiment_results[current_prefix_len]["train_acc_rnn_bk"]), 
                                mean(formula_experiment_results[current_prefix_len]["train_acc_rnn_bk_greedy"])))
                            f.write("test acc next activity:\nRNN+BK:{}\tRNN+BK Greedy:{}\n\n".format(
                                mean(formula_experiment_results[current_prefix_len]["test_acc_rnn_bk"]), 
                                mean(formula_experiment_results[current_prefix_len]["test_acc_rnn_bk_greedy"])))
                            f.write("train DL distance:\nRNN+BK:{}\tRNN+BK Greedy:{}\n".format(
                                mean(formula_experiment_results[current_prefix_len]["train_DL_rnn_bk"]), 
                                mean(formula_experiment_results[current_prefix_len]["train_DL_rnn_bk_greedy"])))
                            f.write("test DL distance:\nRNN+BK:{}\tRNN+BK Greedy:{}\n\n".format(
                                mean(formula_experiment_results[current_prefix_len]["test_DL_rnn_bk"]), 
                                mean(formula_experiment_results[current_prefix_len]["test_DL_rnn_bk_greedy"])))
                            f.write("train sat suffix:\nRNN+BK:{}\tRNN+BK Greedy:{}\n".format(
                                mean(formula_experiment_results[current_prefix_len]["train_sat_rnn_bk"]), 
                                mean(formula_experiment_results[current_prefix_len]["train_sat_rnn_bk_greedy"])))
                            f.write("test sat suffix:\nRNN+BK:{}\tRNN+BK Greedy:{}\n".format(
                                mean(formula_experiment_results[current_prefix_len]["test_sat_rnn_bk"]), 
                                mean(formula_experiment_results[current_prefix_len]["test_sat_rnn_bk_greedy"])))
                            f.write("\n")
